 ...
 */
export function resolveImport(importPath, currentFile, context) {
    return (tryRelativeResolution(importPath, currentFile)
 ...
 */
export function tryRelativeResolution(importPath, currentFile) {
 ...
 */
export function tryAliasResolution(importPath, aliases) {
    if (!aliases)
        return null;
 ...
 */
export function tryFrameworkResolution(importPath, framework) {
    if (!framework)
        return null;
    switch (framework) {
        case 'next':
 ...
 */
export function tryAbsoluteResolution(importPath, basePath) {
 ...
 */
function resolveNextJsImport(importPath) {
 ...
 */
function resolveNuxtImport(importPath) {
    if (importPath.startsWith('~') || importPath.startsWith('@')) {
 ...
 */
function resolveVueImport(importPath) {
    if (importPath.startsWith('@/')) {
        const withoutAlias = importPath.substring(2);
