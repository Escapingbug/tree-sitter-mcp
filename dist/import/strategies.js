 ...
 */
export const RelativeResolver = {
    canResolve: (path) => path.startsWith('./') || path.startsWith('../'),
 ...
 */
export const FrameworkResolver = {
    canResolve: (path, context) => {
    ...
    resolve: (path, context) => {
        if (!context.framework || !context.projectRoot)
            return null;
        const framework = context.framework.toLowerCase();
        switch (framework) {
            case 'next':
 ...
 */
function resolveNextJs(path, projectRoot) {
    if (path.startsWith('@/')) {
 ...
 */
function resolveNuxt(path, projectRoot) {
 ...
 */
function resolveVue(path, projectRoot) {
    if (path.startsWith('@/')) {
        const withoutAlias = path.substring(2);
 ...
 */
function findInAvailableFiles(path, availableFiles) {
