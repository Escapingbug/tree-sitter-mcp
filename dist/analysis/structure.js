...
export function analyzeStructure(nodes) {
...
function buildDependencyGraph(fileNodes) {
...
function resolveImportPath(importPath, currentFile, fileNodes) {
...
function findHighCouplingFiles(dependencies) {
...
function analyzeHtmlStructure(fileNodes) {
    ...
    for (const fileNode of fileNodes) {
        ...
        if (!isHtmlFile || !fileNode.content)
            continue;
        htmlFiles++;
...
function generateStructureFindings(circularDeps, highCouplingFiles, htmlAnalysis) {
    const findings = [];
