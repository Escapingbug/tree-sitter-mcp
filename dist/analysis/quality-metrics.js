 ...
 */
export function calculateMethodLength(node) {
 ...
 */
export function calculateQualityScore(avgComplexity, avgMethodLength, avgParameters) {
    let score = 10;
    if (avgComplexity > 2) {
        if (avgComplexity <= 4) {
 ...
 */
export function adjustScoreForIssues(baseScore, findings, totalMethods, testMethods) {
    ...
    const microFunctionIssues = productionIssues.filter(f => (f.category === QUALITY_CATEGORIES.UNNECESSARY_ABSTRACTION || f.category === QUALITY_CATEGORIES.EXCESSIVE_ABSTRACTION)
        && f.severity === 'critical'
        && Number(f.metrics?.usageCount ?? 0) <= 1);
    const otherCriticalProduction = productionIssues.filter(f => f.severity === 'critical'
        && f.category !== QUALITY_CATEGORIES.UNNECESSARY_ABSTRACTION
        && f.category !== QUALITY_CATEGORIES.EXCESSIVE_ABSTRACTION).length;
