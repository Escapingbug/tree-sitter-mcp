 ...
 */
export function detectMagicValues(node) {
 ...
 */
export function detectDeepNesting(node) {
    if (!node.content || !node.name)
        return [];
    const findings = [];
    const content = node.content;
 ...
 */
export function detectGodClasses(functionNodes) {
    const findings = [];
 ...
 */
export function analyzeFunctionUsage(functionNodes) {
    const usage = new Map();
 ...
 */
export function analyzeMicroFunctionPatterns(functionUsage, functionNodes) {
    const findings = [];
    const shortFunctionsByFile = new Map();
    functionNodes.forEach((node) => {
...
//# sourceMappingURL=quality-patterns.js.map