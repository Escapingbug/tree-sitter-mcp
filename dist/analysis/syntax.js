 ...
 */
export function analyzeSyntaxErrors(project) {
    const findings = [];
    const errorsByType = {};
...
function analyzeProjectFiles(project, findings, errorsByType, filesWithErrors) {
    let totalErrorNodes = 0;
 ...
 */
function findSyntaxErrorsInNode(node, filePath) {
    const errors = [];
 ...
 */
function getErrorContext(node, _filePath) {
    try {
 ...
 */
function determineErrorType(_node, context) {
 ...
 */
function generateErrorMessage(node, errorType) {
    const typeMessages = {
