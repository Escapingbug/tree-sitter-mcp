...
export function analyzeErrors(project) {
    const errors = [];
    const errorsByType = {};
    const errorsByFile = {};
...
function collectAllErrors(project, errors, errorsByType, errorsByFile, filesWithErrors) {
...
function createErrorSummary(errors, filesWithErrors) {
    return {
...
function createErrorMetrics(project, totalErrorNodes, errorsByType, errorsByFile) {
...
function analyzeProjectErrors(project, errors, errorsByType, errorsByFile, filesWithErrors) {
...
function processFileErrors(filePath, fileErrors, errors, errorsByType, errorsByFile, filesWithErrors) {
...
function countProjectErrorNodes(project) {
    let count = 0;
...
export function countErrorNodes(node) {
    let count = 0;
    if (node.hasError) {
        count++;
    }
...
function extractActionableErrors(node, filePath) {
    const errors = [];
    collectErrorsRecursively(node, filePath, errors);
    return errors;
}
function collectErrorsRecursively(node, filePath, errors) {
...
function isActionableErrorNode(node) {
    return node.isMissing || node.type === NODE_PATTERNS.ERROR || node.isExtra;
}
function createActionableError(node, filePath) {
...
function determineErrorType(node) {
    if (node.isMissing)
        return ERROR_TYPES.MISSING;
    if (node.isExtra)
        return ERROR_TYPES.EXTRA;
    return ERROR_TYPES.PARSE_ERROR;
}
function getActionableContext(node) {
    if (!node.parent)
...
function getMissingNodeContext(node) {
...
function identifyContextType(node) {
    const type = node.type;
    if (type.includes(NODE_PATTERNS.FUNCTION)) {
        const name = getFunctionName(node);
        return name ? `function ${name}` : 'anonymous function';
    }
    if (type.includes(NODE_PATTERNS.IF_STATEMENT))
        return 'if statement';
    if (type.includes(NODE_PATTERNS.FOR_STATEMENT))
        return 'for loop';
    if (type.includes(NODE_PATTERNS.WHILE_STATEMENT))
        return 'while loop';
    if (type.includes(NODE_PATTERNS.CLASS))
        return 'class declaration';
    if (type.includes(NODE_PATTERNS.INTERFACE))
...
function generateMissingSuggestion(nodeType) {
    switch (nodeType) {
...
function generateErrorSuggestion(text) {
