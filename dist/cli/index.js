...
import { renderAnalysis, SETUP_TEMPLATE, SETUP_AUTO_SUCCESS_TEMPLATE, SETUP_AUTO_EXISTS_TEMPLATE, SETUP_AUTO_FAILED_TEMPLATE, SETUP_CLAUDE_NOT_FOUND_TEMPLATE } from '../constants/templates.js';
...
async function handleAnalysis(options) {
    const logger = initializeLogger(options.debug ? 'debug' : 'info', options.quiet);
    try {
        ...
        if (options.pathPattern) {
            filteredFindings = result.findings.filter(finding => finding.location.includes(options.pathPattern));
        }
...
async function handleErrors(options) {
    ...
    try {
        ...
        if (options.pathPattern) {
            filteredErrors = result.errors.filter(error => error.file.includes(options.pathPattern));
        }
...
function formatErrorsReport(result) {
    ...
    const errorsByType = {
        ...
        parse_error: errors.filter((e) => e.type === 'parse_error'),
        extra: errors.filter((e) => e.type === 'extra'),
    };
...
async function handleFindUsage(identifier, options) {
    const logger = initializeLogger(options.debug ? 'debug' : 'info', options.quiet);
    try {
...
async function runAutoSetup(logger) {
    try {
...
function handleDefaultAction(options) {
