...
import { renderAnalysis } from '../constants/templates.js';
...
export async function executeAnalysis(directory, options) {
    const logger = getLogger();
...
export async function executeFindUsage(identifier, options) {
    const logger = getLogger();
    const project = createProject({
        directory: options.directory,
        languages: options.languages || [],
