...
export function createProject(config, isSubProject = false) {
    const project = {
        id: generateId(),
        config: {
            ...config,
    ...
    if (!isSubProject) {
        ...
        if (monorepoInfo.isMonorepo) {
            ...
            project.subProjects = monorepoInfo.subProjects.map(subPath => createProject({ ...config, directory: subPath }, true));
        }
    }
    return project;
}
export async function parseProject(project) {
    ...
    try {
        ...
        if (project.subProjects && project.subProjects.length > 0) {
            ...
            const files = await findProjectFiles(project.config.directory, project.config.languages, project.config.ignoreDirs);
...
export async function updateProject(project, changes) {
    const logger = getLogger();
...
export function watchProject(project, onUpdate) {
    const watcher = createFileWatcher(project.config.directory, (changes) => {
...
export function getAllNodes(project) {
    const allNodes = [];
...
function extractAllNodes(fileNode) {
    const nodes = [];
