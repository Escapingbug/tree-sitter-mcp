...
export function detectMonorepo(directory) {
...
export function findSubProjects(directory, maxDepth = 3) {
    const subProjects = [];
...
function detectNpmWorkspaces(directory) {
    try {
...
function detectPnpmWorkspaces(directory) {
    try {
...
function detectLernaWorkspaces(directory) {
    try {
...
export function detectWorkspaces(directory) {
    const workspaces = [];
...
export function findRootProject(directory) {
...
function detectProjectType(projectPath) {
    if (isFile(join(projectPath, 'package.json')))
        return 'npm';
    if (isFile(join(projectPath, 'Cargo.toml')))
        return 'rust';
    if (isFile(join(projectPath, 'go.mod')))
        return 'go';
    if (isFile(join(projectPath, 'pyproject.toml')))
        return 'python';
    if (isFile(join(projectPath, 'pom.xml')))
        return 'java';
    return 'unknown';
}
