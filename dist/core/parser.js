 ...
 */
export function parseContent(content, filePath, language) {
    const extension = extname(filePath);
...
function extractElements(node, content, filePath, language, parent) {
...
function extractFunction(node, content, filePath) {
    try {
...
function extractClass(node, content, filePath) {
    try {
...
function getFunctionName(node, content) {
...
function getClassName(node, content) {
    const nameNode = node.childForFieldName('name');
    if (nameNode) {
        return content.substring(nameNode.startIndex, nameNode.endIndex);
    }
...
function isCallbackArrowFunction(node, content) {
    if (!node.parent)
        return false;
...
function truncateLongLines(content, maxLineLength) {
...
export function getLanguageParser(extension) {
