 ...
 */
export function searchCode(query, nodes, options = {}) {
    const { maxResults = 20, fuzzyThreshold = 30, exactMatch = false, types = [], pathPattern, forceContentInclusion = false, maxContentLines = 150, disableContentInclusion = false, } = options;
    // First pass: collect all matching results without content
    const initialResults = [];
    function collectMatches(currentNodes) {
        for (const node of currentNodes) {
            if (types.length > 0 && !types.includes(node.type))
                continue;
            if (pathPattern && !node.path.includes(pathPattern))
                continue;
 ...
 */
function includeContentInResults(results, options = {}) {
...
function calculateScore(query, node, exactMatch, fuzzyThreshold) {
...
function getMatches(query, node) {
    const matches = [];
    const queryLower = query.toLowerCase();
 ...
 */
export function findUsage(identifier, nodes, options = {}) {
